        ...
        function loadGraph(graphData) {
            ...
            window.addEventListener('keydown', (event) => {
              if (event.key === 'c') {
                ...
                const activeNode = d3.select('circle[active=true]');
                if (!activeNode) console.log("no active node found");
                if (activeNode) {
                  const cx = activeNode.attr('cx');
                  const cy = activeNode.attr('cy');
                  let node = g.append("g").attr("class", "nodes").selectAll(".node");

                 // Translate the graph so that the active node is at the center
                  centerActiveNode(node);         
                  console.log("done")
                }
              }
            });
